...
async function list(req, res) {
    ...
    if (!ctx) return;
    const items = await rules.list(ctx.ownerId);
...
async function update(req, res) {
    ...
    const rule = await rules.update(req.params.id, data, ctx.ownerId);
...
async function remove(req, res) {
    const ctx = await resolveActingOwner(req, res, { write: true });
    if (!ctx) return;
    const rule = await rules.remove(req.params.id, ctx.ownerId);
    if (!rule) return res.status(404).json({ error: 'Not found' });
    res.status(204).end();
}
