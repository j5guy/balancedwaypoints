const { appName, serverUrls, apiSecret } = require('../../config/licensing');

// Tries each configured server in order, falling through to the next only on
// a genuine reachability problem (network error, timeout, 5xx, bad secret) —
// never on a definitive 404/403 answer, which returns immediately instead of
// being treated as "this server is down". If every server in the list fails,
// the caller sees the last one's error, same as the single-server case
// always did — gate.js's revalidate() already treats that as "unreachable"
// and keeps whatever license state was last cached, so an all-down list
// degrades exactly like a single down server always has.
